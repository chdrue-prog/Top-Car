...
document.addEventListener("DOMContentLoaded", () => {
    ...
    function initInventory() {
        ...
        const currentVersion = "1.5"; // Force upgrade to load live parsed cars with images slider and maximum resolution + interiors
...
// ==========================================================================
// YouTube Background Player Controller (Prevents default loop play button flash)
// ==========================================================================
let ytPlayer;

// Globally referenced by the YouTube Iframe Player API script when ready
window.onYouTubeIframeAPIReady = function() {
    ytPlayer = new YT.Player('yt-bg-player', {
        ...
        events: {
            ...
            'onStateChange': (event) => {
                // When video ends (state = 0), seek back to start and play immediately.
                // This bypasses YouTube's loop reload buffer and prevents any play icon or cards flashing.
                if (event.data === 0) {
                    event.target.seekTo(0);
                    event.target.playVideo();
                }
                
                // Fade in the video container ONLY after the video has been actively playing (state = 1) for 1.8 seconds.
                // This completely hides the initial YouTube black screen, loading spinner, and play button icon.
                if (event.data === 1) {
                    setTimeout(() => {
...
// Dynamically inject the YouTube Iframe Player API script
(function() {
